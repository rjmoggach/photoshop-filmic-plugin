...
// originShiftX/Y move the point the kernel is sampled around, in GRID SAMPLES.
// psfFromPupil's discrete grid does not put the PSF's centre of mass exactly on
// the sample the resampler treats as the centre, and the residue is a fraction
// of an output pixel. That is invisible in the kernel itself and very visible
// once a caller subtracts an unconvolved reference from a convolved image: the
// difference of two copies of a picture offset by a tenth of a pixel is its
// edges. Callers that care measure the bias on axis -- where the PSF is
// rotationally symmetric, so any offset is numerical -- and pass it back here.
// squeeze stretches the point spread function horizontally, in image space and
// before the radial rotation. An anamorphic lens has an oval pupil, so its blur
// is oval and its highlights streak sideways -- that, and not the frame's aspect
// ratio, is what makes anamorphic bokeh recognisable.
inline Plane psfAtField(const PsfRings& r, float t, float thetaRad,
                        float samplesPerPixel, int outSize,
                        float originShiftX = 0.0f, float originShiftY = 0.0f,
                        float squeeze = 1.0f) {
    ...
    for (int y = 0; y < outSize; ++y) {
        for (int x = 0; x < outSize; ++x) {
            ...
            const float sq = (squeeze > 1e-3f) ? squeeze : 1.0f;
            const float dx = (float(x) - c) * samplesPerPixel / sq;
            const float dy = (float(y) - c) * samplesPerPixel;
            const float ux =  ct * dx + st * dy;
            const float uy = -st * dx + ct * dy;
            const float sx = gc + ux + originShiftX, sy = gc + uy + originShiftY;
            float a, b;
            if (oversampled) {
