...
inline Plane warpPlane(const Plane& src, const Distortion& d, float K) {
    ...
    for (int y = 0; y < src.h; ++y) {
        for (int x = 0; x < src.w; ++x) {
            ...
            // Black outside the frame, not clamped and not mirrored. Distortion
            // deliberately reaches past the border: clamping smeared one edge
            // pixel into a stripe down each side, and mirroring would invent
            // texture that was never photographed. Black shows where the frame
            // actually ends.
            out.at(x, y) = sampleBilinearBorder(src, dx * f.halfDiag + f.cx,
                                                     dy * f.halfDiag + f.cy);
        }
    }
    return out;
