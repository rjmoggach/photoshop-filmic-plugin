...
struct Params {
    ...
    // Anamorphic squeeze: 1.0 is a spherical lens, 2.0 the classic scope ratio.
    // It widens the point spread function horizontally, giving oval bokeh and
    // sideways-streaking highlights.
    float psfSqueeze = 1.0f;

    // How much of the vignette model to apply, and which way. 1.0 is the full
    // physical falloff, 0 none, and a negative value brightens the corners
    // instead. Kept separate from VignetteParams because that struct describes
    // the LENS, while this is how far the user wants to push it.
