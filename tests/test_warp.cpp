...
TEST_CASE("a warp off the edge loses energy rather than duplicating it") {
    // A block sitting hard against the frame border, warped so that samples fall
    // outside it.
    //
    // This test used to assert the opposite: that clamping DUPLICATED the edge
    // and so inflated total energy. That was an accurate description of what the
    // code did, and what the code did was wrong -- every out-of-frame sample
    // returned the same boundary pixel, smearing it into a stripe down the side
    // of the picture. Outside the frame there is no picture, and the honest
    // answer is black.
    Plane src(65, 65);
    for (int y = 50; y < 65; ++y) for (int x = 50; x < 65; ++x) src.at(x, y) = 1.0f;
    const Plane out = warpPlane(src, Distortion{}, -0.1f);

    // Still a convex combination of up to four samples, so nothing brightens.
    ...
    // Energy may only be LOST off the edge, never invented.
    CHECK(b <= a + 1e-3);
    // But the block is mostly still in frame, so it must not vanish either --
    // "returns black everywhere" would satisfy the check above on its own.
    CHECK(b > 0.5 * a);
}
