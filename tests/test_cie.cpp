...
TEST_CASE("ybar peaks near 555nm") {
    ...
    CHECK(arg == doctest::Approx(555.0f).epsilon(0.02).scale(0));
    CHECK(best == doctest::Approx(1.0f).epsilon(0.05).scale(0));
}
...
TEST_CASE("equal energy spectrum is illuminant E, chromaticity one third") {
    ...
    CHECK(sum.x / t == doctest::Approx(1.0f / 3.0f).epsilon(0.02).scale(0));
    CHECK(sum.y / t == doctest::Approx(1.0f / 3.0f).epsilon(0.02).scale(0));
}

TEST_CASE("Rec2020 matrices are mutual inverses") {
    const RGB in{0.3f, 0.7f, 0.2f};
    const RGB out = xyzToRec2020(rec2020ToXyz(in));
    CHECK(out.r == doctest::Approx(in.r).epsilon(1e-4).scale(0));
    CHECK(out.g == doctest::Approx(in.g).epsilon(1e-4).scale(0));
    CHECK(out.b == doctest::Approx(in.b).epsilon(1e-4).scale(0));
}
...
TEST_CASE("Rec2020 white maps to a neutral chromaticity") {
    ...
    CHECK(w.x / t == doctest::Approx(0.3127f).epsilon(0.01).scale(0));  // D65
    CHECK(w.y / t == doctest::Approx(0.3290f).epsilon(0.01).scale(0));
}
