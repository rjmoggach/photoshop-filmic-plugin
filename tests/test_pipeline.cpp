...
TEST_CASE("vignetting reproduces the model's radial curve") {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        // Mirror render()'s own shaping: the physical falloff is close to linear
        // in field height, and the render weights its LOSS by t^2 so the middle
        // stays flat and the corner falls away -- which is how real vignetting
        // behaves, and why it is modelled as a polynomial in r squared.
        const float phys = optics::naturalFalloff(p.vignette, t) * psfRatio;
        const float expected = 1.0f - p.vignetteAmount * (1.0f - phys) * t * t;
        CAPTURE(i);
...
TEST_CASE("a grid too small for any kernel draws no blur instead of failing") {
    // psfGrid=8 spans less than a 3x3 kernel at every sampled wavelength, which
    // says the PSF is smaller than one pixel. A sub-pixel PSF has nothing to
    // convolve, so the band passes through untouched.
    Params p = nullParams();
    p.doPsf = true;
    p.psfGrid = 8;
    p.psfRings = 2;
    ...
    Image out;
    CHECK_NOTHROW(out = render(toImage(flatField(48, 48, 0.5f)), p, table()));
    for (size_t i = 0; i < out.px.size(); i += 3)
        CHECK(out.px[i] == doctest::Approx(0.5f).epsilon(0.02).scale(0));
}

TEST_CASE("a coarser pixel pitch shrinks the kernel instead of failing") {
    // This is how a preview works: it renders a proxy by telling the model its
    // pixels are larger, so that a blur covers the same FRACTION of the frame as
    // it will at full size. That shrinks how wide a kernel the grid can cover, in
    // exact proportion.
    //
    // It used to throw, and the whole preview failed with "psfKernel (7) exceeds
    // what psfGrid=256 covers at 408nm" the moment the band count rose and
    // shorter wavelengths came into play. The convolution normalises every kernel
    // to unit sum now, so a narrower window costs a little of the PSF's outer
    // tail and nothing else.
    for (float pitchScale : {1.0f, 2.0f, 4.0f, 8.0f, 16.0f}) {
        CAPTURE(pitchScale);
        Params p = nullParams();
        p.doPsf = true;
        p.bands = 7;                       // short wavelengths are the tight case
        p.petzval = 1.5f;
        p.pixelPitchUm *= pitchScale;

        Image out;
        CHECK_NOTHROW(out = render(toImage(flatField(48, 48, 0.5f)), p, table()));

        // Not throwing is not enough: the result has to stay usable.
        float lo = 1e9f, hi = -1e9f;
        for (size_t i = 0; i < out.px.size(); i += 3) {
            lo = std::min(lo, out.px[i]);
            hi = std::max(hi, out.px[i]);
        }
        CHECK(lo == doctest::Approx(0.5f).epsilon(0.05).scale(0));
        CHECK(hi == doctest::Approx(0.5f).epsilon(0.05).scale(0));
    }
}
