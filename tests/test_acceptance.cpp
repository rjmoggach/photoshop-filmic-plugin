...
TEST_CASE("ACCEPTANCE: stopping down removes mechanical vignetting from the render") {
    ...
    // render() weights the vignette's LOSS by t^2 (see pipeline.hpp): a straight
    // ramp from centre to corner does not look like a lens, which is why real
    // vignetting is conventionally modelled as a polynomial in r squared.
    const float t94 = 0.94f;
    const float phys94 = optics::naturalFalloff(p.vignette, t94);
    const float expected94 = 1.0f - p.vignetteAmount * (1.0f - phys94) * t94 * t94;
    CHECK(stopped == doctest::Approx(expected94).epsilon(0.05).scale(0));
}
